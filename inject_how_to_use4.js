...
function processDirectory(dir) {
    ...
    for (const file of files) {
        ...
        if (stat.isDirectory()) {
            ...
            if (content.includes('{/* FAQs */}') && !content.includes('How to Use This Calculator') && !content.includes('How to Use ──')) {
                const match = content.match(/^([ \t]*)\{\/\* FAQs \*\/\}/m);
                if (match) {
                    const indent = match[1];
                    const indentedBlock = howToUseBlock.split('\n').map((line, i) => i === 0 ? line.trimStart() : (line ? indent + line.trimStart() : '')).join('\n');
                    
                    content = content.replace(/^([ \t]*)\{\/\* FAQs \*\/\}/m, indent + indentedBlock);
