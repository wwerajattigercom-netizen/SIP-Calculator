...
export default function SipVsFdPpfRd() {
  ...
  const chartData = {
    labels: ['Total Invested', 'Bank RD', 'PPF', 'Equity DCA'],
    datasets: [
      {
        label: 'Corpus Value ($)',
  ...
  const jsonLd = {
    ...
    "mainEntity": [
      {
        "@type": "Question",
        "name": "Which is better: DCA, PPF, or FD/RD?",
        "acceptedAnswer": {
          "@type": "Answer",
      ...
      {
        "@type": "Question",
        "name": "Can equity DCA beat PPF and FD?",
        "acceptedAnswer": {
          "@type": "Answer",
            ...
            <div className="lg:col-span-7 glass-panel p-5 lg:p-6 flex flex-col h-[500px]">
              ...
              <div className="mt-4 bg-[rgba(5,150,105,0.05)] border border-[#059669]/20 p-4 rounded-xl text-center">
                <p className="text-foreground text-sm">
                  By choosing <strong className="text-[var(--color-accent)]">Equity DCA</strong> over <strong className="text-[#C4993C]">PPF</strong>, 
                  you generate an additional <strong className="text-[#059669] text-base">{fmtUSD(results.sipFv - results.ppfFv)}</strong> in wealth over {years} years.
                </p>
              </div>
            </div>
