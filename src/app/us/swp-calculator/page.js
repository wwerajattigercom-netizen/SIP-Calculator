...
const jsonLd = {
  ...
  '@graph': [
    ...
    },
    {
      ...
      step: [
        { '@type': 'HowToStep', name: 'Enter Total Corpus', text: 'Enter the total accumulated corpus available for withdrawal. Range: $10,000 to $5 Million.' },
      ...
      mainEntity: [
        {
          ...
          acceptedAnswer: {
            '@type': 'Answer',
            text: 'An SWP allows you to withdraw a fixed amount of money regularly (usually monthly) from your mutual fund investments. It is the opposite of a DCA. It is commonly used by retirees to generate a steady pension-like income from their accumulated corpus.',
          },
        },
        {
          '@type': 'Question',
...
const HOW_TO_STEPS = [
  { step: '1', title: 'Enter Total Corpus', desc: 'The accumulated amount you have available for withdrawals. Range: $10,000 to $5 Million.' },
...
const FAQS = [
  {
    ...
    a: 'An SWP allows you to withdraw a fixed amount of money regularly (usually monthly) from your mutual fund investments. It is the opposite of a DCA. It is commonly used by retirees to generate a steady pension-like income from their accumulated corpus.',
  },
  {
...
export default function SWPCalculatorPage() {
  ...
  const pieData = {
    ...
    datasets: [{
      ...
      backgroundColor: [(typeof isDark !== 'undefined' && isDark ? '#1A73E8' : '#1B3A5C'), '#C4993C'],
  ...
  const lineData = {
    ...
    datasets: [
      { label: 'Corpus Balance', data: yearlyData.map(d => d.balance), borderColor: (typeof isDark !== 'undefined' && isDark ? '#1A73E8' : '#1B3A5C'), backgroundColor: 'transparent', tension: 0.4, pointRadius: 0, pointHitRadius: 10, fill: false },
    ],
  };
  const lineOptions = {
    responsive: true, maintainAspectRatio: false,
    interaction: { mode: 'index', intersect: false },
    plugins: { legend: { display: false }, tooltip: { callbacks: { label: (ctx) => `${ctx.dataset.label}: $${ctx.raw.toLocaleString('en-US')}` } } },
    scales: {
            ...
            <div className="lg:col-span-5 glass-panel p-4 lg:p-5 relative">
              ...
              <div className="mt-4 pt-1 space-y-2">
                ...
                <Link
                  ...
                  className="flex items-center gap-2 text-[#1B3A5C] dark:text-[#1A73E8] text-xs hover:text-foreground transition-colors group"
                >
                  ...
                  {[{ key: 'pie', label: 'Pie Chart' }, { key: 'line', label: 'Line Chart' }].map(({ key, label }) => (
                    <button
                      ...
                      className={`flex-1 py-1.5 rounded-md text-xs font-medium transition-all ${
                        chartTab === key ? 'bg-[#1B3A5C] dark:bg-[#1A73E8] text-white shadow-lg' : 'text-gray-500 dark:text-gray-400 hover:text-foreground'
                      }`}
                    >{label}</button>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 relative z-10 mt-auto">
                  <div className="bg-black/5 dark:bg-white/5 rounded-lg p-2 flex flex-col justify-center">
                    <div className="flex items-center text-foreground text-[11px] mb-0.5 font-semibold font-medium">
                      <div className="w-2 h-2 rounded-full bg-[#1B3A5C] dark:bg-[#1A73E8] mr-1.5" />Withdrawn
                    </div>
                    <div className="text-sm font-extrabold text-[#1B3A5C] dark:text-[#1A73E8]">{formatCurrency(totalWithdrawn)}</div>
      ...
      <div className="max-w-6xl w-full mx-auto px-4 pb-16 space-y-10 mt-6">
                      ...
                      >
                        <td className="py-2.5 px-4">
                          <div className="flex items-center gap-2">
                            <span className={`font-semibold ${isLast ? 'text-[#1B3A5C] dark:text-[#1A73E8]' : 'text-gray-600 dark:text-gray-400'}`}>
                              Year {year}
                            </span>
                            {isDepleted && !isLast && (
                            ...
                            {isLast && !isDepleted && (
                              <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-[#1B3A5C] dark:bg-[#1A73E8] text-white">
                                Final
                              </span>
                            )}
                          </div>
        ...
        <section id="how-to-use" aria-label="How to use the SWP calculator">
          <div className="flex items-center gap-3 mb-6">
            <div className="bg-[#1B3A5C] dark:bg-[#1A73E8] bg-opacity-20 border border-[#1B3A5C] dark:border-[#1A73E8] p-2 rounded-xl">
              <Info className="w-5 h-5 text-[#1B3A5C] dark:text-[#1A73E8]" />
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {HOW_TO_STEPS.map(({ step, title, desc }) => (
              <div key={step} className="glass-panel p-5 flex gap-4">
                <div className="flex-shrink-0 w-9 h-9 rounded-full bg-[#1B3A5C] dark:bg-[#1A73E8] flex items-center justify-center text-white shadow-sm font-bold text-sm">
                  {step}
                </div>
                <div>
        ...
        <section id="faq" aria-label="SWP calculator frequently asked questions">
          <div className="flex items-center gap-3 mb-6">
            <div className="bg-[#1B3A5C] dark:bg-[#1A73E8] bg-opacity-20 border border-[#1B3A5C] dark:border-[#1A73E8] p-2 rounded-xl">
              <HelpCircle className="w-5 h-5 text-[#1B3A5C] dark:text-[#1A73E8]" />
            </div>
            ...
            {FAQS.map(({ q, a }, i) => (
              <div key={i} className="glass-panel overflow-hidden">
                ...
                >
                  ...
                  <ChevronDown
                    className={`w-4 h-4 text-[#1B3A5C] dark:text-[#1A73E8] flex-shrink-0 transition-transform duration-200 ${openFaq === i ? 'rotate-180' : ''}`}
                  />
                </button>
                {openFaq === i && (
        ...
        <section id="swp-formula" aria-label="SWP formula and methodology" className="mt-2">
          <div className="glass-panel p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Formula */}
              <div>
                <h3 className="text-[#1B3A5C] dark:text-[#1A73E8] font-semibold text-sm mb-3 uppercase tracking-wider">The Concept</h3>
              ...
              {/* Worked example */}
              <div>
                <h3 className="text-[#1B3A5C] dark:text-[#1A73E8] font-semibold text-sm mb-3 uppercase tracking-wider">Worked Example (Month 1)</h3>
          ...
          <div className="glass-panel p-6 bg-gradient-to-r from-[rgba(27,58,92,0.1)] to-[rgba(27,58,92,0.08)]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <Link href="/us/dca-calculator" className="flex items-center gap-3 glass-panel p-3 hover:bg-[rgba(27,58,92,0.15)] transition-all group rounded-xl">
                <div className="bg-[#1B3A5C] dark:bg-[#1A73E8] bg-opacity-20 border border-[#1B3A5C] dark:border-[#1A73E8] p-2 rounded-lg flex-shrink-0">
                  <Calculator className="w-4 h-4 text-[#1B3A5C] dark:text-[#1A73E8]" />
                </div>
                <div>
                  <p className="text-foreground font-semibold text-sm group-hover:text-[#1B3A5C] dark:text-[#1A73E8] transition-colors">Step-Up DCA</p>
              ...
              <Link href="/us/lumpsum-calculator" className="flex items-center gap-3 glass-panel p-3 hover:bg-[rgba(27,58,92,0.15)] transition-all group rounded-xl">
                <div className="bg-[#1B3A5C] dark:bg-[#1A73E8] bg-opacity-20 border border-[#1B3A5C] dark:border-[#1A73E8] p-2 rounded-lg flex-shrink-0">
                  <Layers className="w-4 h-4 text-[#1B3A5C] dark:text-[#1A73E8]" />
                </div>
                <div>
                  <p className="text-foreground font-semibold text-sm group-hover:text-[#1B3A5C] dark:text-[#1A73E8] transition-colors">Lumpsum Returns</p>
                  <p className="text-gray-500 dark:text-gray-400 text-xs">One-time investments</p>
                </div>
              </Link>

              <Link href="/us/target-amount-calculator" className="flex items-center gap-3 glass-panel p-3 hover:bg-[rgba(27,58,92,0.15)] transition-all group rounded-xl">
