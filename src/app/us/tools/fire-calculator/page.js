...
export default function FireCalculatorPage() {
  ...
  const accentColor = isDark ? '#1A73E8' : '#1B3A5C';
    ...
    useEffect(() => {
        ...
        if (months > 0 && r > 0) {
            // DCA calculation formula: P = FV / [ ( ( (1 + r)^n - 1 ) / r ) * (1 + r) ]
              ...
              <div>
                ...
                <div className="mt-12 glass-panel p-8 text-center bg-gradient-to-r from-[rgba(27,58,92,0.1)] to-[rgba(27,58,92,0.05)] w-full rounded-3xl border border-[var(--color-accent)]/10">
                    ...
                    <div className="flex flex-wrap justify-center gap-3">
                        ...
                        <Link href="/us/tools/roth-ira-calculator" className="inline-flex items-center gap-2 border border-[var(--color-accent)]/40 text-[var(--color-accent)] hover:border-[var(--color-accent)] px-5 py-2.5 rounded-xl text-sm font-medium transition-all">
                            Roth IRA Calculator
                        </Link>
                    </div>
                </div>
