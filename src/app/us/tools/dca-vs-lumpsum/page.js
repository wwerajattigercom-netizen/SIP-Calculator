...
const SCENARIO_META = {
  ...
  bear: {
    ...
    color: '#991B1B',
    icon: TrendingDown,
    desc: 'Market drops for ~40% of the period, then recovers. DCA buys cheap units during the crash.',
    tag: 'Crash + Recovery',
  },
  volatile: {
    ...
    color: '#059669',
    icon: Minus,
    desc: 'Alternating surges and dips every 6 months. DCA benefits from buying more units during dips.',
    tag: 'High Swings',
  },
};
...
export default function SipVsLumpsumPage() {
  ...
  const { chartConfig, finalSip, finalLumpsum } = useMemo(() => {
    ...
    // DCA: each month buy monthlySip / NAV[month] units
    let sipUnits = 0;
    const labels = [];
    const lumpsumData = [];
    const sipData = [];
    ...
    for (let year = 1; year <= duration; year++) {
      ...
      // accumulate DCA units up to this month
  ...
  const winner = finalLumpsum > finalSip ? 'Lumpsum' : 'DCA';
          ...
          <div className="glass-panel p-7 relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-gray-500 dark:text-gray-400 text-sm leading-relaxed">
                Should you invest your money all at once (Lumpsum) or spread it over time (DCA)?
              ...
              <div className="glass-panel p-6 flex flex-col gap-5 h-full">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className={`p-5 rounded-2xl border transition-all ${
                    ...
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-gray-500 dark:text-gray-400 text-xs font-semibold uppercase tracking-wider">DCA</span>
                      {winner === 'DCA' && (
                ...
                <div className="bg-[var(--panel-bg)] border border-[#E8E4DF] rounded-xl p-4 text-center">
                  <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
                    ...
                    <strong style={{ color: winner === 'DCA' ? 'var(--color-accent)' : '#C4993C' }}>{winner}</strong>{' '}
                    wins by{' '}
                    ...
                    options={{
                      ...
                      scales: {
                        y: {
                          ...
                          ticks: { color: '#6B7280', callback: (v) => formatToShortWords(v) },
                        },
                        x: {
                          border: { display: false },
                          grid: { display: false },
              ...
              <table className="w-full text-sm text-left">
                <thead>
                  <tr className="border-b border-black/5">
                    ...
                    <th className="py-2 text-[var(--color-accent)] font-semibold">DCA</th>
                  </tr>
                </thead>
