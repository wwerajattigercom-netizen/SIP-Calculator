...
export default function BuyVsRentPageUS() {
  ...
  const [buyingClosingCostsPercent, setBuyingClosingCostsPercent] = useState(3.0);
  const [sellingClosingCostsPercent, setSellingClosingCostsPercent] = useState(6.0); // Realtor commissions
  ...
  const results = useMemo(() => {
    ...
    const n = loanTerm * 12;
    const monthlyMortgage = loanAmount > 0 ? (loanAmount * r * Math.pow(1 + r, n) / (Math.pow(1 + r, n) - 1)) : 0;
    ...
    for (let year = 1; year <= loanTerm; year++) {
      ...
      for (let month = 1; month <= 12; month++) {
        ...
        const totalBuyerMonthlyCost = monthlyMortgage + monthlyPropertyTax + monthlyMaintenance;
    ...
    const totalBuyerSunkCosts = totalInterestPaid + totalPropertyTaxes + totalMaintenance + buyingClosingCosts + (propertyValue * Math.pow(1 + propertyAppreciation / 100, loanTerm) * (sellingClosingCostsPercent / 100));
    ...
    return {
      ...
      totalMaintenance,
      buyingClosingCosts,
      sellingCostsFinal: propertyValue * Math.pow(1 + propertyAppreciation / 100, loanTerm) * (sellingClosingCostsPercent / 100)
    };
  }, [propertyValue, downPaymentPercent, mortgageRate, loanTerm, propertyAppreciation, propertyTaxPercent, maintenancePercent, buyingClosingCostsPercent, sellingClosingCostsPercent, monthlyRent, rentEscalation, equityReturns]);
            ...
            <div className="glass-panel p-6 bg-[var(--background)] rounded-2xl">
              ...
              <div className="space-y-5">
                ...
                <InputSlider
                  ...
                  value={sellingClosingCostsPercent}
                  min={0}
                  max={10}
                  step={0.5}
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm text-[#6B7280] dark:text-[#8B95A5]">
                  ...
                  <tbody>
                    ...
                    <tr className="border-b border-[var(--panel-border)]">
                      <td className="px-4 py-3 font-medium text-[var(--foreground)]">Mortgage Interest</td>
                    ...
                    <tr className="border-b border-[var(--panel-border)]">
                      <td className="px-4 py-3 font-medium text-[var(--foreground)]">HOA & Maintenance</td>
        ...
        <div className="mt-12 glass-panel p-8 text-center bg-gradient-to-r from-[rgba(27,58,92,0.1)] to-[rgba(27,58,92,0.05)] w-full rounded-3xl border border-[var(--color-accent)]/10">
            ...
            <div className="flex flex-wrap justify-center gap-3">
                <Link href="/us/tools/dca-for-house-down-payment" className="inline-flex items-center gap-2 bg-[var(--color-accent)] hover:bg-[var(--color-accent-hover)] text-white shadow-sm px-5 py-2.5 rounded-xl text-sm font-semibold transition-all">
                    Down Payment DCA <ArrowRight className="w-4 h-4" />
                </Link>
