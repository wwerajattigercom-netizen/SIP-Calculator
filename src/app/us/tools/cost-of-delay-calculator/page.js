...
export default function CostOfDelayCalculatorPage() {
  ...
  const lineOptions = {
    ...
    scales: {
      ...
      y: { grid: { color: 'rgba(0,0,0,0.05)' }, ticks: { color: '#8B95A5', callback: (v) => v >= 1e6 ? `$${(v/1e6).toFixed(1)}M` : `$${(v/1e3).toFixed(0)}K` } },
    },
  };
    ...
    <>
      ...
      <main className="py-6 px-2 md:px-4 flex flex-col items-center">
        <div className="max-w-6xl w-full mx-auto">
          <CalculatorTabs />
          <Breadcrumb items={[{ label: 'Tools', href: '/us' }, { label: 'Cost of Delay Calculator' }]} />
              ...
              <div className="glass-panel p-6 text-center border-l-4 border-l-[#991B1B] bg-[#fffaf5]">
                ...
                <div className="bg-black/5 dark:bg-white/5 rounded-lg p-3 inline-block">
                  <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 dark:text-gray-400">
                    To catch up, you'd need to invest <strong className="text-[var(--color-returns)]">${Math.round(extraSIP + monthlySIP).toLocaleString('en-US')}</strong>/month instead.
                  </p>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    That's an extra <strong className="text-[var(--color-returns)]">${Math.round(extraSIP).toLocaleString('en-US')}</strong> every month!
                  </p>
                </div>
              </div>
          ...
          <div className="mt-8 glass-panel p-5 lg:p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-6">
              ...
              <div className="bg-black/5 dark:bg-white/5 rounded-lg p-3">
                ...
                <div className="text-lg font-bold text-[var(--color-returns)]">+{fmtUSD(extraSIP)}/mo</div>
              </div>
            </div>
          </div>
