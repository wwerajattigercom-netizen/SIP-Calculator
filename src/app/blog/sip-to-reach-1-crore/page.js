        ...
        <article className="max-w-3xl w-full mx-auto space-y-8">
          ...
          <div className="glass-panel p-7 relative overflow-hidden">
            ...
            <div className="relative z-10">
              <span className="text-[10px] uppercase tracking-widest text-[#a78bfa] font-semibold mb-3 block">Guide · SIP Planning</span>
              <h1 className="text-2xl md:text-3xl font-extrabold text-white leading-tight mb-3">
                How Much SIP to Reach <span className="text-gradient">₹1 Crore</span> in 10, 15 &amp; 20 Years?
              </h1>
              ...
              <p className="text-gray-500 text-xs mt-3">By <span className="text-gray-300">Rajat</span> · Updated 2024</p>
            </div>
          </div>

          {/* Quick Answer Box */}
          <div className="glass-panel p-5 border border-[rgba(139,92,246,0.3)]">
            <h2 className="text-white font-bold mb-3 flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-[#a78bfa]" /> Quick Answer — SIP for ₹1 Crore at 12% p.a.
            </h2>
            <div className="grid grid-cols-3 gap-3 text-center">
              {[
                ...
                <div key={label} className="bg-[rgba(139,92,246,0.1)] rounded-xl p-3">
                  <p className="text-gray-400 text-xs mb-1">{label}</p>
                  <p className="text-white font-bold text-sm">{sip}</p>
                </div>
              ))}
            </div>
          ...
          <div className="glass-panel p-6">
            <h2 className="text-xl font-bold text-white mb-2">Monthly SIP Required for ₹1 Crore</h2>
            <p className="text-gray-400 text-xs mb-4">All figures assume no step-up, lump sum ₹0, SIP invested at the start of each month.</p>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-[#c4b5fd] border-b border-white/10 text-xs uppercase tracking-wider">
                ...
                <tbody className="text-gray-300">
                  {TABLE_DATA.map(({ years, r10, r12, r15 }) => (
                    <tr key={years} className="border-b border-white/5">
                      <td className="py-2.5 pr-4 font-medium">{years} Years</td>
                      <td className="py-2.5 pr-4 text-right">{fmt(r10)}</td>
                      <td className="py-2.5 pr-4 text-right text-[#a78bfa] font-semibold">{fmt(r12)}</td>
                      <td className="py-2.5 text-right text-[#22C55E]">{fmt(r15)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {/* Step-Up SIP Section */}
          <div className="glass-panel p-6">
            <h2 className="text-xl font-bold text-white mb-2">Step-Up SIP: Lower Starting SIP, Same ₹1 Crore Goal</h2>
            <p className="text-gray-400 text-sm mb-4 leading-relaxed">
              A <strong className="text-white">step-up SIP</strong> increases your monthly SIP by a fixed percentage each year —
              matching your salary hikes. The result: you start with a lower SIP but still reach the same goal.
              Below is a comparison for a <strong className="text-white">12% return, 10-year</strong> target of ₹1 Crore:
            </p>
            <div className="space-y-3">
              {STEPUP_DATA.map(({ label, sip, total, invested, gain }) => (
                <div key={label} className="flex flex-col sm:flex-row sm:items-center gap-2 p-4 rounded-xl bg-[rgba(255,255,255,0.03)] border border-white/5">
                  <p className="text-white text-sm font-medium sm:w-52 flex-shrink-0">{label}</p>
                  <div className="flex gap-4 text-xs flex-wrap">
                    <span className="text-gray-400">Starting SIP: <span className="text-white font-bold">{fmt(sip)}/mo</span></span>
                    <span className="text-gray-400">Final corpus: <span className="text-[#22C55E] font-bold">{total}</span></span>
                    <span className="text-gray-400">Total invested: <span className="text-gray-300">{invested}</span></span>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* The power of time */}
          <div className="glass-panel p-6">
            <h2 className="text-xl font-bold text-white mb-3">The Biggest Factor: Time</h2>
            <p className="text-gray-400 text-sm leading-relaxed mb-4">
              The table above shows that extending your SIP from 10 to 20 years at 12% reduces
              your required SIP from <strong className="text-white">₹43,000 to just ₹8,000/month</strong> — a 5x reduction.
              This is the power of compounding. The longer you invest, the less you need to invest each month.
            </p>
            <div className="bg-[rgba(139,92,246,0.08)] border border-[rgba(139,92,246,0.2)] rounded-xl p-4 text-sm text-gray-400">
              ...
              Waiting until 35 requires ₹43,000/month for the same goal. <strong className="text-white">15 years of delay costs ₹35,000/month extra.</strong></p>
            </div>
          </div>
          ...
          <div className="glass-panel p-5 border border-amber-400/20">
            <div className="flex items-start gap-2 text-sm text-gray-400">
              ...
              <p>
                <strong className="text-amber-400">Disclaimer:</strong> These figures assume a constant annual return rate,
                ...
                <Link href="/disclaimer" className="text-[#a78bfa] hover:underline ml-1">Read our full methodology →</Link>
              </p>
            </div>
          </div>

          {/* CTA */}
          <div className="glass-panel p-6 text-center bg-gradient-to-r from-[rgba(139,92,246,0.1)] to-[rgba(59,130,246,0.08)]">
            <h2 className="text-lg font-bold text-white mb-2">Calculate Your Exact SIP Amount</h2>
            <p className="text-gray-400 text-sm mb-4">Use our free interactive calculators to get precise projections with step-up, inflation, and lump sum.</p>
            <div className="flex flex-wrap justify-center gap-3">
              ...
              <Link href="/target-amount-calculator" className="inline-flex items-center gap-2 border border-[#8b5cf6]/40 text-[#a78bfa] hover:border-[#8b5cf6] px-5 py-2.5 rounded-xl text-sm font-medium transition-all">
