      ...
      <main className="py-8 px-2 md:px-4 flex flex-col items-center">
        <article className="max-w-3xl w-full mx-auto space-y-8">
          ...
          <div className="glass-panel p-7 relative overflow-hidden">
            <div className="absolute inset-0 bg-gradient-to-br from-[rgba(34,197,94,0.07)] to-transparent pointer-events-none" />
            <div className="relative z-10">
              <span className="text-[10px] uppercase tracking-widest text-[#22C55E] font-semibold mb-3 block">Guide · Return Metrics</span>
              <h1 className="text-2xl md:text-3xl font-extrabold text-white leading-tight mb-3">
                <span className="text-[#a78bfa]">CAGR</span> vs <span className="text-[#22C55E]">XIRR</span> vs Absolute Return — What's the Difference?
              </h1>
          ...
          {/* Quick answer */}
          <div className="glass-panel p-5 border border-[rgba(139,92,246,0.25)]">
            <h2 className="text-white font-bold mb-3">The One-Line Answer</h2>
            <div className="space-y-2 text-sm">
              {[
                { metric: 'Absolute Return', when: 'Quick total gain % — no time context', color: 'text-amber-400' },
                { metric: 'CAGR',            when: 'Annualised return for lumpsum investments', color: 'text-[#a78bfa]' },
                { metric: 'XIRR',            when: 'Annualised return for SIP / multiple cash flows', color: 'text-[#22C55E]' },
                ...
                <div key={metric} className="flex items-start gap-2">
                  ...
                  <span className="text-gray-400"><span className={`font-bold ${color}`}>{metric}:</span> {when}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-4">
            <h2 className="text-xl font-bold text-white">Deep Comparison</h2>
            {COMPARISON.map(({ metric, formula, useFor, bestFor, limitation, example, color, border }) => (
              <div key={metric} className={`glass-panel p-5 border ${border}`}>
                ...
                <div className="space-y-2 text-sm">
                  <div className="bg-[rgba(0,0,0,0.3)] rounded-lg px-3 py-2 font-mono text-xs text-gray-300">{formula}</div>
                  {[
                    ...
                    <div key={label} className="flex gap-2">
                      <span className="text-gray-600 w-20 flex-shrink-0 text-xs pt-0.5">{label}:</span>
                      <span className="text-gray-400 text-xs leading-relaxed">{val}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="glass-panel p-6">
            <h2 className="text-xl font-bold text-white mb-3">Which Metric Do Groww and Zerodha Use?</h2>
            <div className="space-y-3 text-sm text-gray-400 leading-relaxed">
              <p>
                When you view your SIP portfolio on <strong className="text-white">Groww</strong> or <strong className="text-white">Zerodha Coin</strong>,
                the return percentage shown is <strong className="text-[#22C55E]">XIRR</strong> — not CAGR, not absolute return.
              ...
              <p>
                When you look at a mutual fund's <strong className="text-white">historical performance</strong> (e.g., "Axis Bluechip Fund 5-year returns"),
                that number is <strong className="text-[#a78bfa]">CAGR</strong> — assuming a single lumpsum investment at the start.
              </p>
              <div className="bg-[rgba(139,92,246,0.08)] border border-[rgba(139,92,246,0.2)] rounded-xl p-4">
                <p className="text-white font-semibold mb-1">⚠️ Common Mistake</p>
          ...
          <div className="glass-panel p-6">
            <h2 className="text-xl font-bold text-white mb-3">Why Can&apos;t You Use CAGR for SIP Returns?</h2>
            ...
            <div className="text-xs text-gray-500 bg-[rgba(255,255,255,0.02)] border border-white/5 rounded-lg p-3">
              <p className="text-gray-300 mb-1">Rule of thumb:</p>
          ...
          <div className="glass-panel p-5 border border-amber-400/20">
            <div className="flex items-start gap-2 text-sm text-gray-400">
              ...
              <p>
                <strong className="text-amber-400">Disclaimer:</strong> This article is for educational purposes only.
                Mutual fund investments are subject to market risks. Past returns are not indicative of future performance.
                <Link href="/disclaimer" className="text-[#a78bfa] hover:underline ml-1">Read our methodology →</Link>
              </p>
            </div>
          </div>

          {/* CTA */}
          <div className="glass-panel p-6 text-center bg-gradient-to-r from-[rgba(34,197,94,0.06)] to-[rgba(139,92,246,0.08)]">
            <h2 className="text-lg font-bold text-white mb-2">Calculate CAGR of Any Investment</h2>
            <p className="text-gray-400 text-sm mb-4">Use our free CAGR Calculator — supports lumpsum and lumpsum+SIP combined mode.</p>
            <div className="flex flex-wrap justify-center gap-3">
              <Link href="/cagr-calculator" className="inline-flex items-center gap-2 bg-[#8b5cf6] hover:bg-[#7c3aed] text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-all">
