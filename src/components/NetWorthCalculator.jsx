...
export default function NetWorthCalculator() {
  ...
  const [liquidCash, setLiquidCash] = useState(500000);
  const [otherAssets, setOtherAssets] = useState(500000);
  ...
  const chartData = {
    labels: ['Real Estate', 'Investments (MF/Stocks)', 'Bank & Cash', 'Other Assets'],
    datasets: [
      {
    ...
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start w-full">
      ...
      <div className="xl:col-span-7 flex flex-col gap-6">
        ...
        <div className="glass-panel p-5 relative flex flex-col gap-3 rounded-2xl border-t-4 border-t-[#059669]">
          ...
          <InputSlider label="Investments (Mutual Funds, Stocks, PF)" value={investments} onChange={setInvestments} min={0} max={20000000} step={50000} prefix="₹" />
          <InputSlider label="Bank Balance & Cash" value={liquidCash} onChange={setLiquidCash} min={0} max={10000000} step={10000} prefix="₹" />
          <InputSlider label="Other Assets (Gold, Vehicles)" value={otherAssets} onChange={setOtherAssets} min={0} max={10000000} step={50000} prefix="₹" />
        </div>
        ...
        <div className="glass-panel p-5 relative flex flex-col gap-3 rounded-2xl border-t-4 border-t-[#991B1B]">
          ...
          <InputSlider label="Car Loan Outstanding" value={carLoan} onChange={setCarLoan} min={0} max={10000000} step={50000} prefix="₹" />
          <InputSlider label="Personal / Education Loans" value={personalLoan} onChange={setPersonalLoan} min={0} max={10000000} step={50000} prefix="₹" />
          <InputSlider label="Credit Card Debt" value={creditCard} onChange={setCreditCard} min={0} max={2000000} step={10000} prefix="₹" />
        </div>
            ...
            <div className="relative z-10 flex flex-col">
                ...
                <div className="grid grid-cols-2 gap-4 mt-2 pt-4 border-t border-white/20">
                    <div>
                        ...
                        <span className="text-[#34d399] font-bold text-lg">{formatCurrency(results.totalAssets)}</span>
                    </div>
                    <div>
                        <span className="text-white/70 text-xs font-medium block mb-1">Total Liabilities</span>
                        <span className="text-[#fca5a5] font-bold text-lg">{formatCurrency(results.totalLiabilities)}</span>
                    </div>
                </div>
            </div>
        </div>
